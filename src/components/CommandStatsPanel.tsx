...
export function CommandStatsPanel({ target }: { target: SelectedTarget }) {
  ...
  const baseColumns: TableColumnsType<CmdStat> = [
    { title: "Command", dataIndex: "command", width: 220, ellipsis: { showTitle: false }, render: (c: string) => <TruncatedText className="mono" style={{ fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>{c}</TruncatedText> },
