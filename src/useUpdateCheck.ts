...
const CACHE_KEY = "super-redis-update-check";
const CACHE_TTL = 4 * 60 * 60 * 1000; // 4 hours
const PENDING_UPDATE_KEY = "super-redis:install-update-on-launch";
...
export function useUpdateCheck(currentVersion: string, enabled: boolean = true) {
  ...
  useEffect(() => {
    ...
    let pending = false;
    try {
      pending = localStorage.getItem(PENDING_UPDATE_KEY) === "1";
    } catch {
      /* ignore */
    }
    if (!pending) doCheck();
